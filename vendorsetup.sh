...
{
        local kernel_path=$(find device/sprd -name "$TARGET_PRODUCT.mk" | xargs grep  "KERNEL_PATH *:=" | grep -o "kernel[49]\.[0-9]*")
        if [ -z "$kernel_path" ] ; then
		include_files=$(find device/sprd -name "$TARGET_PRODUCT.mk" -exec grep "include" {} \; | grep -o "device/sprd/.*")
		for include_file in $include_files
		do
			kernel_path_line=$(grep  "KERNEL_PATH *:=" $include_file)
			if [ -z "$kernel_path_line" ] ; then
				continue
			fi
			kernel_path=$(echo $kernel_path_line | grep -o "kernel[49]\.[0-9]*")
...
function kheader()
{
	...
	bash bsp/build/make_for_androidq.sh kheader
}
...
function kmconfig()
{
	...
	command make -C $T/$kernel_path O=$OUT/obj/KERNEL ARCH=$(get_arch_name) menuconfig
}
...
function kuconfig()
{
	...
	bash bsp/build/make_for_androidq.sh kuconfig
}
...
function kdconfig()
{
	...
	mkdir -p $OUT/obj/KERNEL
	kernel_path=$(get_kernel_path)
	command make -C $T/$kernel_path O=$OUT/obj/KERNEL ARCH=$(get_arch_name) $KERNEL_DEFCONFIG
}
...
function kcconfig()
{
	...
	mkdir -p $OUT/obj/KERNEL
	kernel_path=$(get_kernel_path)
	command make -C $T/$kernel_path O=$OUT/obj/KERNEL ARCH=$_ARCH $KERNEL_DEFCONFIG
	cr=`diff $OUT/obj/KERNEL/.config $T/$kernel_path/arch/$_ARCH/configs/$KERNEL_DEFCONFIG`
	if [ "$cr" == "" ];then
...
function kclean()
{
	...
	kernel_path=$(get_kernel_path)
	command make -C $T/$kernel_path O=$OUT/obj/KERNEL ARCH=$(get_arch_name) mrproper
}
...
function uclean()
{
	...
	command make -C $T/u-boot O=$OUT/obj/u-boot mrproper
}
...
function umk()
{
	...
	command make -C $T/u-boot O=$OUT/obj/u-boot $*
...
function uconfig()
{
	...
	command make -C $T/u-boot O=$OUT/obj/u-boot $(get_build_var UBOOT_DEFCONFIG)_config
}
